...
# Timers
timers = {"statistics": 0, "loading": 0, "total": 0}
...
if args.binfmt:
    dat = pd.DataFrame(
        data=np.reshape(np.fromfile(args.fname), (-1, 6)),
        columns=["x", "y", "z", "u", "v", "w"],
    )
...
u2 = np.mean(dat["u"] ** 2)
dudx2 = np.mean(
    np.gradient(dat["u"].values.reshape((res, res, res), order="F"), dx, axis=0) ** 2
)
...
divu = (
    np.gradient(dat["u"].values.reshape((res, res, res), order="F"), dx, axis=0)
    + np.gradient(dat["v"].values.reshape((res, res, res), order="F"), dx, axis=1)
    + np.gradient(dat["w"].values.reshape((res, res, res), order="F"), dx, axis=2)
)
...
logging.info("    KE = {0:f} (u:{1:f}, v:{2:f}, w:{3:f})".format(KE, KEu, KEv, KEw))
