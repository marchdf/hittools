...
class VelocityTestCase(unittest.TestCase):
    ...
    def test_gaussian_filter(self):
        ...
        npt.assert_almost_equal(np.linalg.norm(filtered.U[0]),
                                7.388369839861693)
        npt.assert_almost_equal(np.linalg.norm(filtered.U[1]),
                                14.776739679723407)
        npt.assert_almost_equal(np.linalg.norm(filtered.U[2]),
                                22.165109519585087)
        npt.assert_almost_equal(np.linalg.norm(filtered.Uf[0]),
                                945.71133950229762)
        npt.assert_almost_equal(np.linalg.norm(filtered.Uf[1]),
                                1891.422679004595)
